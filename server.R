...
server <- function(input, output, session) {
  ...
  output$visualize <- renderTable({
    #mirbasegff3<-readGFF("/home/guillemyllabou/Documents/mirPlot_Shiny/filterMirBase/data/zma.gff3")
  ...
  observeEvent(input$run, {
    ...
    withProgress(message = 'Progress', value = 0, {    
      ...
      for(i in 1:length(mirbasegff3asranges_precs)){# for each precursor
        # print(i)
        incProgress(1/n)
